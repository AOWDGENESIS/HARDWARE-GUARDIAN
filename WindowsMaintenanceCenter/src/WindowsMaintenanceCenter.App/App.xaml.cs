...
namespace WindowsMaintenanceCenter.App;
    ...
    protected override async void OnStartup(StartupEventArgs e)
    {
        ...
        try
        {
            ...
            if (!OperatingSystem.IsWindows())
            {
                ...
                MessageBox.Show(
                    $"Windows Maintenance Center requires Windows. Current platform: {Environment.OSVersion}.",
        ...
        catch (Exception ex)
        {
            LogFatal(ex);
            MessageBox.Show(
                $"Windows Maintenance Center could not start.{Environment.NewLine}{Environment.NewLine}{ex.GetType().Name}: {ex.Message}",
                "Windows Maintenance Center",
                MessageBoxButton.OK,
                MessageBoxImage.Error);
            Shutdown(1);
