...
| Gate 8 Localization | M37 | Deutsch und Englisch sind vollständig (847 Schlüssel je Sprache, 0 fehlend, 0 tot, Platzhalter je Schlüssel über alle Sprachen gleich). **Japanisch und Russisch fehlen weiterhin vollständig.** Der Weg dorthin ist jetzt aber frei und geprüft: `LanguageCatalog` liest die angebotenen Sprachen aus der Assembly (also aus den vorhandenen Katalogdateien), `LanguagePreference` kennt `Japanese` und `Russian`, die Auswahl zeigt lokalisierte Namen statt roher Kennungen, und die Prüfung vergleicht Platzhalter über alle Sprachen samt Selbsttest (Mutation 18). Ein neuer Katalog wird damit beim Einbau automatisch mitgeprüft | `src/WindowsMaintenanceCenter.Core/Resources/` (`de.json`, `en.json`; `ja.json` und `ru.json` fehlen) | ja-JP und ru-RU übersetzen (je 847 Schlüssel), einbauen, dann Kapitel 63 auf der Zielmaschine prüfen. Eine muttersprachliche Durchsicht bleibt ein eigener offener Punkt, auch nach dem Einbau |
