...
| Gate 2 Unit | `test-results/ci/run-35704157557-1/` (TRX und `test.log`): **370 ausgeführt, 370 bestanden, 0 fehlgeschlagen, 0 übersprungen** auf der CI-Maschine, Commit `fd11038`; `scripts/test.ps1` verlangt TRX und mindestens 370 Fälle. **(Offen und nicht als bestanden zu lesen:) der Baum enthält inzwischen 22 weitere Fälle** - 3 Lokalisierung, 19 Ein-Klick-Wartung - die **nie ausgeführt wurden**; sie sind geschrieben und geprüft, aber nicht gemessen, und die CI kann seit 2026-09-22 kontobedingt nicht starten | **bestanden für den gemessenen Stand (CI-Umgebung)**; die 22 neuen Fälle sind NOT VERIFIED |
...
| P0-MUSS (M34/M35) | M34, M35 | Zustandsjournal und Recovery-Engine sind auf Codeebene umgesetzt und getestet (`FileStateJournal`, `RecoveryEngine`); der Nachweis eines echten Absturzes mit Wiederanlauf auf einer Windows-VM (REC-01/02) steht noch aus | `src/WindowsMaintenanceCenter.Infrastructure/Persistence/FileStateJournal.cs`, `src/WindowsMaintenanceCenter.Core/Services/RecoveryEngine.cs` | REC-01/02 auf der Ziel-VM fahren |
| P0-MUSS (M02 BitLocker) | M02 | BitLocker ist in Kapitel 8 ein MUSS. Die Abfrage ist jetzt umgesetzt (`BitLockerReader`, eigene Prüfung im Windows-Bericht, 11 Testfälle), aber **kein Laufwerk wurde je gelesen**: das braucht Windows und ein verschlüsseltes oder unverschlüsseltes NTFS-Laufwerk | `src/WindowsMaintenanceCenter.Hardware/Wmi/BitLockerReader.cs`, `tests/WindowsMaintenanceCenter.Tests/BitLockerReadingTests.cs`; `test-results/integration/` leer für diesen Punkt | Auf der Ziel-VM lesen und die Werte (Laufwerk, Schutzstatus, Umwandlungsstatus, Methode) als Nachweis ablegen |
| P0-MUSS (M12 CHKDSK) | M12 | Dateisystem-Integrität via CHKDSK ist umgesetzt (`RunFileSystemCheckAsync`, `InterpretChkdskOutput`, `M12-E-002` Neustartbedarf, `M12-R-001` WinRE-Recovery-Pfad, 5 Testfälle), aber **auf keiner echten Maschine ausgeführt** | `src/WindowsMaintenanceCenter.Windows/WindowsHealthService.cs`, `tests/WindowsMaintenanceCenter.Tests/IntegrityCheckTests.cs` | CHKDSK /scan auf Windows-VM ausführen und Beleg erfassen |
| P0-MUSS (M18 Scans) | M18 | Quick Scan und Full Scan sind umgesetzt (`RunDefenderScanAsync`, PowerShell-Katalog, `M18-F-003` Statusnachprüfung, Deaktivierungsverbot `M18-S-001/S-002`), aber **auf keiner echten Maschine ausgeführt** | `src/WindowsMaintenanceCenter.Windows/WindowsHealthService.cs`, `tests/WindowsMaintenanceCenter.Tests/DefenderScanTests.cs` | Defender-Scan auf Windows-VM ausführen und Beleg erfassen |
| P0-MUSS (M04/M22) | M04, M22 | M04: Freier Speicher vor/nach Cleanup wird gemessen (`SystemFreeSpaceBefore/After`, M04-F-004); M22: Dynamische Restore-Point-Bezeichnung und Validierung (`RP_VALIDATED`, M22-F-002) umgesetzt, aber **auf keiner echten Maschine ausgeführt** | `MaintenanceService.cs`, `BackupService.cs`, Tests | Auf echter Windows-VM mit Systemwiederherstellung und Dateisystem prüfen |
| P0-MUSS (M38/M39) | M38, M39 | Passwörter und Dokumentinhalte dürfen nicht ins Protokoll (M38-S-001/S-002). Die Redaktion ist eingebaut und getestet (`SensitiveDataGuard` in technischem Protokoll und Audiodatei, 12 Testfälle), **aber auf keiner Maschine mit einem echten Lauf nachgewiesen**; was der Wächter nicht kann - einen als Satz formulierten Inhalt - ist in `docs/STATUS.md` ausdrücklich benannt. `M39-S-005` (Diagnoseexport prüfen) hat jetzt ein Werkzeug (`Scan`), aber keinen gelaufenen Export | `src/WindowsMaintenanceCenter.Core/Security/SensitiveDataGuard.cs`; `test-results/security/` leer | Diagnoseexport auf der Maschine erzeugen, mit `Scan` prüfen und das Ergebnis ablegen |
