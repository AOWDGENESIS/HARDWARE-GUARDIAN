...
| P0-Umfang | M31, M32, M33 | Der Admin Worker führt jetzt genau eine registrierte Aktion erhöht aus (Kapitel 37/38); ungeprüft bleibt er trotzdem: kein Sicherheitstest (Gate 4) ist gelaufen, die Reparaturwerkzeuge sind bewusst `Allowed = false`, und ihre Exit-Codes wurden nie auf einer echten Maschine gemessen | `src/WindowsMaintenanceCenter.Infrastructure/Platform/AdminWorker.cs`, `SystemActionCatalog.cs` | Gate 4 auf isolierter VM, Freigabe-/Backup-Tor vor die Reparaturwerkzeuge |
