...
| INS-CI | **Installationszyklus auf der CI-Maschine** (Gate 7, Kapitel 62/71): Silent-Installation → Layout, Version und SHA-256 gegen den Bau → Start → Nutzungssonde (Datenordner + Logdatei) → Exit-Code → Silent-Deinstallation → Restprüfung (Eintrag, Dateien, Verknüpfung weg; Daten bewusst behalten) → portables Artefakt in leerem Ordner (Daten daneben) | der Zyklus läuft durch und findet nichts: 20 Kriterien PASS, 0 FAIL, Ergebnis `PASSED` im Bericht; offen bleiben Reparatur, Upgrade, Neustart, weitere Laufwerke und „USE" (Kapitel 62) — der Bericht nennt sie als offene Punkte | `test-results/installer/20260922T082333Z-INS-CI/` (`report.json`, `report.txt`, `steps.txt`, `logs/`, `startup-log-tail.txt`) | PASSED (CI-Umgebung; Zielmaschine bleibt für die offenen Punkte nötig) |
