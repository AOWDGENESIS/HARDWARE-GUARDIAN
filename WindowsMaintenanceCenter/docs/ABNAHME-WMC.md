...
| Ebene | Zustand |
| --- | --- |
| Build (Gate 1) | **bestanden auf der CI-VM** (Lauf `35704157557`, Commit `fd11038`: 15 Projekte inklusive WPF, 0 Fehler; Lauf `35694300454` war der erste grüne Bau) |
| Unit-Tests (Gate 2) | **bestanden für den gemessenen Stand auf der CI-VM**: 370 Fälle ausgeführt, 0 fehlgeschlagen (Lauf `35704157557`, Commit `fd11038`); **22 Fälle sind seitdem dazugekommen (3 Lokalisierung, 19 Ein-Klick-Wartung) und nie gelaufen - NOT VERIFIED**; TRX und `summary.txt` in `test-results/unit/20260922T062120Z-359-of-359/` |
...
| Lokalisierung (Gate 8) | vier Kataloge vorhanden (de, en, ja, ru, je 939 Schlüssel, maschinell geprüft); **kein Sprachlauf und keine Screenshots auf einer Maschine, keine muttersprachliche Prüfung → `BLOCKED`** |
...
| --- | --- | --- | --- |
| M00 Core / Foundation | P0 | Shell, Navigation, Konfiguration lesen/schreiben, Absturzschutz für kaputte Konfiguration; **`M00-L-003` Japanisch und `M00-L-004` Russisch sind vorhanden** (vier Kataloge, Sprachwechsel über die Einstellung) | `M00-F-003` sauberes Beenden und `M00-O-001` Start ohne Internet nicht belegt |
...
| M02 System Discovery | P0 | CPU, RAM, GPU, Laufwerke, Mainboard, BIOS/UEFI, TPM, Secure Boot, Windows, Netzwerk, Defender, Firewall, Restore Points, **BitLocker** | **BitLocker ist umgesetzt** (`BitLockerReader` liest das dokumentierte `Win32_EncryptableVolume` in `Root\CIMV2\Security\MicrosoftVolumeEncryption`; eine eigene Prüfung im Windows-Bericht nennt Laufwerke ohne Schutz, laufende Umwandlungen und nicht gemeldete Zustände getrennt; 11 Testfälle; **nie auf einem echten Laufwerk gelesen**). Offen: `M02-E-001` Protokollierung nicht erreichbarer Quellen im Lauf belegen |
...
| M04 Cleanup Engine | P0 | Ablauf SCAN→CLASSIFY→CALCULATE→DISPLAY→APPROVAL→BACKUP→EXECUTE→VALIDATE vorhanden, Kategorien mit Sicherheitsklasse, Trockenlauf erzwungen; **`M04-F-004` freier Speicher vor und nach dem Lauf wird auf der Festplatte nachgemessen** (`SystemFreeSpaceBefore`, `SystemFreeSpaceAfter`, Evidence) | Persönliche Ordner (Desktop, Docs, Bilder, Videos) sind im Katalog ausgeschlossen (M04-S-001…S-005); Nachweis auf echter VM offen |
...
| M12 Repair Engine | P0 | DISM CheckHealth/ScanHealth/RestoreHealth, SFC /verifyonly, **CHKDSK /scan und /f umgesetzt** (`RunFileSystemCheckAsync`, `InterpretChkdskOutput`, `M12-E-002` Neustartbedarf, `M12-R-001` dokumentierter WinRE-Recovery-Pfad, 5 Testfälle) | Nachweis mit echtem Werkzeuglauf auf Zielmaschine offen |
...
| M18 Security Center | P0 | Defender-Status, Echtzeitschutz, Firewallprofile, Signaturstand; **`M18-F-001` Quick Scan und `M18-F-002` Full Scan umgesetzt** (`RunDefenderScanAsync`, PowerShell-Katalog, `M18-F-003` Statusabfrage nach Scan; `M18-S-001/S-002` Deaktivierungsverbot durch Testfälle gesichert) | Lauf auf Windows-Zielmaschine offen |
...
| M22 Restore Points | P0 | Erstellung, **dynamische Vorgangsbezeichnung (`WMC-{Kind}-{OpId}`) und Validierung nach Erstellung (`M22-F-002`, `RP_VALIDATED`)** umgesetzt | Lauf auf Windows-Zielmaschine mit aktiver Systemwiederherstellung offen |
...
| M27 One-Click Maintenance | P0 | **umgesetzt, nicht auf einer Maschine gelaufen, nicht aus der Oberfläche bedient**: `OneClickMaintenanceService` führt genau die acht Phasen DISCOVERY→DIAGNOSTIC→PLAN→APPROVAL→BACKUP→EXECUTION→VALIDATION→REPORT in dieser Reihenfolge aus, hält Zustandsmaschine, Journal und Protokoll mit, misst freien Speicher vorher und nachher (fehlende Messung bleibt UNKNOWN, nie 0) und schreibt den Bericht; eigene Seite `OneClickView` mit Abwahl je Kategorie (M27-S-002), Risikoanzeige je Phase und je Element (M27-S-003), Warten auf die Freigabe und Abbruchknopf | **Auf einer Maschine nicht gelaufen** (kein Windows hier, CI kontobedingt nicht startbar); `M27-R-001` ist nur auf Codeebene belegt: die Unterbrechung endet als `CANCELLED` mit genannter Operation, die Wiederaufnahme selbst ist Sache der Recovery-Engine und dort noch nicht auf einer Maschine nachgewiesen |
...
| M37 Localization | P1 | **Vier Kataloge liegen bei: de, en, ja, ru - je 961 Schlüssel, 0 fehlend, Platzhalter über alle Sprachen gleich** (Prüfwerkzeug und vier Testfälle sichern das), Auswahl zeigt übersetzte Sprachnamen, eine Sprache wird nur angeboten, wenn ihr Katalog im Bau ist | **Die Kataloge sind maschinell erstellt und nicht von Muttersprachlern geprüft**; Datums-/Zahlenformate und der sichtbare Nachweis je Sprache (Kapitel 63, LOC-01/LOC-02 mit Screenshots) fehlen weiterhin |
| M38 Logging | P0 | Dateilog, Audit-Protokoll, Exit-Codes; **Geheimnisse werden vor dem Schreiben entfernt** (`SensitiveDataGuard`: Schlüsselwortwerte, Kommandazeilen-Passwörter, BitLocker-Wiederherstellungskennwörter, private Schlüsselblöcke; technisches Protokoll und Audiodatei sind angeschlossen, ein `redacted`-Kennzeichen nennt den Vorgang) | Level TRACE…CRITICAL als steuerbare Stufen sind da; was bleibt: **der Nachweis auf einer Maschine**, dass in einem echten Lauf nichts Sensibles im Log landet, und die ausdrückliche Feststellung, was der Wächter **nicht** kann - Dateiinhalte, die als normaler Satz im Log stehen, erkennt er nicht; M38-S-002 stützt sich deshalb darauf, dass die Anwendung keine Dateiinhalte in Meldungen schreibt |
| M39 Privacy | P0 | Telemetrie standardmäßig aus, keine Cloudpflicht; **Diagnoseexport-Prüfung auf sensible Daten ist als `SensitiveDataGuard.Scan` vorhanden** (nennt Regel, Position und maskierten Auszug) und im Hinweis der Einstellungsseite sichtbar | `M39-S-005` auf einer Maschine belegen: Export erzeugen, prüfen, Ergebnis ablegen |
...
(de, en, ja, ru, je 939 Schlüssel); Symmetrie, Platzhaltergleichheit und die Einbettung in den Bau
