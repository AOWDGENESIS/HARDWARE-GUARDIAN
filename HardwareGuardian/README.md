...
```
src/HardwareGuardian.Core             domain, contracts, services, localisation resources
src/HardwareGuardian.Infrastructure   paths, registry, processes, persistence, HTTP, security, backup, rollback
src/HardwareGuardian.Hardware         WMI-based provider for real hardware
src/HardwareGuardian.Sensors          ACPI, performance counter, storage and vendor sensor providers
src/HardwareGuardian.Drivers          driver inventory and PnP problem analysis
src/HardwareGuardian.Bios             firmware assessment and firmware file inspection
src/HardwareGuardian.Windows          DISM, event log, Defender, Windows Update, services
src/HardwareGuardian.Maintenance      scan, plan, dry run, execute; software and workload inventory
src/HardwareGuardian.Manufacturer     official source adapters and the update centre
src/HardwareGuardian.Security         security report aggregation
src/HardwareGuardian.Reporting        JSON / TXT / HTML reports
src/HardwareGuardian.Simulation       clearly labelled simulation fixture (reference machine)
src/HardwareGuardian.Diagnostics      the diagnostic modules used by the scan pipeline
src/HardwareGuardian.App              WPF shell: DI root, MVVM, themes, runtime DE/EN
tests/HardwareGuardian.Tests          automated tests (xUnit v3)
