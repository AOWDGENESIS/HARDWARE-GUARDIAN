...
Drei Punkte sind keine Implementierungslücken, sondern Entscheidungen, die den Bestand betreffen:

| Punkt | Spezifikation | Bestand | Wirkung |
| --- | --- | --- | --- |
| Produktname | „Windows Maintenance Center", Artefakte `WindowsMaintenanceCenter-*` | Projekt, Namespaces, Ressourcen, Installer und Skripte heißen `HardwareGuardian` | Umbenennung betrifft Lösung, 15 Projekte, Namensräume, Ressourcen, Installer, Skripte, Dokumente. **Entscheidung des Auftraggebers erforderlich** (jetzt umbenennen oder erst nach Funktionsfertigkeit) |
| Datenhaltung | SQLite als Kernbestandteil (Kapitel 100) und eigenes Modul für Datenbanksicherheit (Kapitel 82: Schema-Version, Migration, Integritätsprüfung, Backup, Recovery) | Konfiguration und Audit als JSON-Dateien, kein SQLite | Neue Abhängigkeit und neues Modul. **Entscheidung erforderlich**: JSON beibehalten (einfacher, prüfbar) oder auf SQLite wechseln |
