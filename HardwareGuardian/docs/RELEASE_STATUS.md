...
TESTS:   0 ausgeführt (206 geschrieben, 0 kompiliert)
PASSED:  0
FAILED:  0
BLOCKED: 206
...
| Gate 2 Unit | M00-M48 | 206 Testfälle geschrieben, nie ausgeführt | `tests/HardwareGuardian.Tests`, TRX fehlt | `scripts/test.ps1` (verlangt TRX und mindestens 120 Fälle), Ergebnis nach `test-results/unit/` |
...
| Gate 7 Installer | M40, M41 | Inno-Setup-Definition nie kompiliert, nie ausgeführt | `installer/HardwareGuardian.iss`, `test-results/installer/` leer | Kapitel 62/74 auf sauberer VM |
| Gate 8 Localization | M37 | Japanisch und Russisch fehlen vollständig (742 Schlüssel je Sprache existieren nur für Deutsch und Englisch) | `src/HardwareGuardian.Core/Resources/`, `test-results/localization/` leer | ja-JP und ru-RU übersetzen, dann Kapitel 63 prüfen |
...
| P0-Umfang | M31 | M31-S-001/S-002 verletzt: die Anwendung startet als Ganzes erhöht neu, statt eine registrierte Aktion privilegiert auszuführen | `src/HardwareGuardian.Infrastructure/Platform/ElevationService.cs` | Admin Worker und Action Registry nach Kapitel 37/38 umsetzen |
