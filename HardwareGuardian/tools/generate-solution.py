#!/usr/bin/env python3
"""Writes HardwareGuardian.sln from the projects that actually exist.
...
Usage:
    python3 tools/generate-solution.py            # write HardwareGuardian.sln
...
SOLUTION = ROOT / "HardwareGuardian.sln"
...
def project_guid(relative: str) -> str:
    return "{" + str(uuid.uuid5(uuid.NAMESPACE_URL, "hardwareguardian/" + relative.lower())).upper() + "}"


def folder_guid(name: str) -> str:
    return "{" + str(uuid.uuid5(uuid.NAMESPACE_URL, "hardwareguardian/folder/" + name.lower())).upper() + "}"
...
def main() -> int:
    ...
    if args.check:
        if current != content:
            print("HardwareGuardian.sln is out of date - run tools/generate-solution.py")
            return 1
        print("HardwareGuardian.sln is up to date")
        return 0
