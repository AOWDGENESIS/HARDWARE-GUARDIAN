<#
.SYNOPSIS
    Builds Hardware Guardian from a clean checkout.
...
$solution = Join-Path $root 'HardwareGuardian.sln'
$app = Join-Path $root 'src/HardwareGuardian.App/HardwareGuardian.App.csproj'
...
Write-Host "Hardware Guardian $version ($revision) - $Configuration $RuntimeIdentifier" -ForegroundColor Green
...
try {
    ...
    if (-not $SkipPublish) {
        ...
        Write-Host "published: $(Join-Path $output 'HardwareGuardian.exe')" -ForegroundColor Green
    }
}
finally {
    Pop-Location
