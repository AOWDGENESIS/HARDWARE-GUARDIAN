    ...
    public async Task<BackupRecord> CreateAsync(BackupRequest request, IProgress<ProgressSnapshot>? progress, CancellationToken cancellationToken)
    {
        ...
        if (request.Risk >= RiskLevel.High && OperatingSystem.IsWindows())
        {
            ...
            var result = await RunPowerShellAsync(
                "try { Checkpoint-Computer -Description 'HardwareGuardian' -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop; $p = Get-ComputerRestorePoint | Sort-Object SequenceNumber -Descending | Select-Object -First 1; 'RP=' + $p.SequenceNumber + '|' + $p.CreationTime } catch { 'RP_ERROR=' + $_.Exception.Message }",
